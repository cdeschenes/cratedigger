...
**Genre tag filter / re-score.** Tags are always fetched when `DISCOVER_TAG_OVERLAP > 0` or `DISCOVER_SIMILARITY_MODE=tags`. In `lastfm` mode, candidates are filtered by minimum shared-tag count (`DISCOVER_TAG_OVERLAP`). In `tags` mode, candidates are re-scored by Jaccard genre-tag similarity; only the top `DISCOVER_TAG_TOP_N` tags per artist (by Last.fm weight) are used for scoring, and any candidate scoring below `DISCOVER_MIN_JACCARD` is dropped. Tags on a blocklist don't count toward any score. The blocklist covers platform noise ("spotify", "heard on pandora"), user-behavior tags ("seen live", "favourite"), generic noise words ("music", "cool"), and era/decade tags ("90s", "80s", "2000s", "retro", etc.). Era tags are excluded because they describe *when* an artist was active, not what they sound like — two artists sharing only a decade tag are not musically similar.
